...
* After each generation or validation/fix step, `PROGRESS.md` must be updated with checklist/task completion status before moving to the next step or phase.

* The `PROGRESS.md` update must mark completed checklist items, note any incomplete or blocked items, and record whether the current phase is ready to continue.

* Every important implementation decision taken during planning, generation, or validation must be documented in `DESIGN-DECISIONS.md`.

* Each decision recorded in `DESIGN-DECISIONS.md` should briefly state:
  * what was decided,
  * why it was decided,
  * which document or requirement justified it,
  * and which files or services were affected.

* If the generated output required a deviation, correction, or assumption, that change must also be recorded in `DESIGN-DECISIONS.md`.

* A phase must not be considered complete until the implementation artifacts, `PROGRESS.md` checklist updates, and corresponding `DESIGN-DECISIONS.md` decision updates are finished.
...
- [x] If the service persists state, it uses its own dedicated persistence layer
- [x] Dockerfile builds and container starts without errors
- [x] `.env.example` is complete
- [x] No requirements from the brief are missing; no extra requirements added
- [x] Unit tests cover the core business logic of the service
...
- [x] Remove frontend runtime artifacts from Docker Compose and repository source layout
...
- [x] All application endpoints require JWT; only `/auth/register` and `/auth/login` are public application endpoints, with operational health/metrics endpoints exposed for Docker and Prometheus (M-24, M-25)
...
- [x] Database schemas, migrations, document models, or index/schema initializers are present for services with persistence
