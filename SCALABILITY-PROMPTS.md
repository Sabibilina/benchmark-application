...
Use the current repository as the implementation baseline. Use `ARCHITECTURE.md` as the source of truth for required behavior, system boundaries, allowed implementation technologies. If any conflict appear, stop and report the conflict instead of guessing.
...
Update PROGRESS.md only to mark checklist/task completion status for this planning step.

## 2. Generate

Now take the approved cost-aware scaling plan and implement the first serious version of it in this repository.

Use `ARCHITECTURE.md`, `SCALABILITY.md`, and the current repository. Preserve required backend behavior, service boundaries, Docker Compose runtime, observability, load generation, and documented workload assumptions. Make only changes that follow from the approved plan.
...
- Keep benchmark evidence collectable so cost and performance can be compared after implementation.
...
Update PROGRESS.md only to mark checklist/task completion status for this generation step.

## 3. Validate/Fix

Review the generated scalability and cost-aware changes against `ARCHITECTURE.md`, `SCALABILITY.md`, the approved scaling plan, and the current repository.
...
Update PROGRESS.md only to mark checklist/task completion status for this validation step.
